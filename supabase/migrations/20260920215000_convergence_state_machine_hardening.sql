...
AS $function$
declare
 tid uuid; t public.teams%rowtype; s public.convergence_team_state%rowtype; g public.convergence_game_control%rowtype;
 r public.convergence_route_items%rowtype; cp public.convergence_checkpoints%rowtype;
 n integer; ts timestamptz:=clock_timestamp(); next_step text;
begin
 ...
 select count(*) into n from public.convergence_anticheat_events where team_id=tid and event_type='ACTION_ATTEMPT' and created_at>=ts-interval '10 seconds';
 if n>=10 then return jsonb_build_object('ok',false,'reason','rate_limited','retry_after_seconds',10); end if;
 ...
 if p_action='ACK_HANDIN' and s.current_step='HAND_IN' then
   next_step:='STICKER_1';
   update public.convergence_team_state set current_step=next_step,last_action_at=ts,updated_at=ts where team_id=tid;
   perform public.convergence_audit_event(tid,'HANDIN_REVEALED',g.current_stage,'HAND_IN',jsonb_build_object('next_step',next_step,'action_timestamp',ts));
 ...
 if p_action='ACK_RIDDLE' then
   if s.current_step='RIDDLE_4' then next_step:='STICKER_4';
   elsif s.current_step='RIDDLE_7' then next_step:='STICKER_7';
   else return jsonb_build_object('ok',false,'reason','invalid_step'); end if;
   update public.convergence_team_state set current_step=next_step,last_action_at=ts,updated_at=ts where team_id=tid;
 ...
 if p_action='ACK_CLUE_8' and s.current_step='CLUE_8' then
   next_step:='CLUE_9';
   if not exists(select 1 from public.convergence_route_items where team_id=tid and step_key='CLUE_9' and published=true) then
     return jsonb_build_object('ok',false,'reason','route_not_configured');
   end if;
   update public.convergence_team_state set current_step=next_step,last_action_at=ts,updated_at=ts where team_id=tid;
   perform public.convergence_audit_event(tid,'CLUE_8_COMPLETED',g.current_stage,'CLUE_8',jsonb_build_object('next_step',next_step,'action_timestamp',ts));
   return jsonb_build_object('ok',true,'next_step',next_step,'timestamp',ts,'server_now',ts);
 end if;
 ...
 if p_action='CHECKPOINT_QR' then
   select * into cp from public.convergence_checkpoints where stage=case when s.current_step='CHECKPOINT_1_QR' then 3 else 6 end and track_id=t.track_id and active=true;
   ...
   select * into cp from public.convergence_checkpoints where stage=case when s.current_step='CHECKPOINT_1_CODE' then 3 else 6 end and track_id=t.track_id and active=true;
   ...
   if cp.id is not null and cp.snippet_answer_hash=extensions.crypt(trim(p_value),cp.snippet_answer_hash) then
     if s.current_step='SNIPPET_1' then
       ...
       if n=10 then
         ...
         from public.teams tx where x.team_id=tx.id and tx.track_id=t.track_id and x.stage3_rank is null and x.status not in('ELIMINATED','WINNER');
         if (select count(*) from (select tx.track_id from public.convergence_team_state x join public.teams tx on tx.id=x.team_id where x.stage3_rank is not null group by tx.track_id having count(*)>=10) closed_tracks)>=4 then
   ...
   select * into r from public.convergence_route_items where team_id=tid and step_key='CLUE_9' and published=true;
