...
import {
  ...
  getConvergenceTeamHistory, getPositionAssignments, getTransitionRiddles, importConvergenceConfig, moveConvergenceTeamPosition, restoreConvergenceTeam, reopenConvergenceCheckpoint,
...
export function ConvergenceStudioScreen() {
  ...
  return <OrganizerLayout title="Convergence Studio">
    <div className="space-y-5">
      ...
      {tab==='data'&&<section className="bg-[#111] border border-gray-800 rounded p-5 space-y-5"><h3 className="text-white font-semibold">Data, History & Import/Export</h3><div className="border border-red-900/40 bg-red-950/10 rounded p-4 space-y-3"><div><h4 className="text-white text-sm">Emergency Organizer Overrides</h4><p className="text-xs text-gray-500 mt-1">Use only for live-event corrections. Every override is logged server-side.</p></div><div className="grid md:grid-cols-2 gap-2">{teams.filter(t=>['A','B','C','D'].includes(t.track_id)).map(t=><div key={t.id} className="border border-gray-900 rounded p-2 flex items-center gap-2"><div className="min-w-0 flex-1"><div className="text-xs text-white truncate">{t.name} · {t.track_id}</div><div className="text-[9px] text-gray-600">Original {String(t.original_team_number??'—')} · Current {String(t.current_position??'—')} · {t.status}</div></div><input id={`override-${t.id}`} defaultValue={t.current_position??''} type="number" min={1} className="w-16 bg-[#171717] border border-gray-800 rounded px-2 py-1 text-xs text-white"/><button disabled={busy} onClick={()=>{const el=document.getElementById(`override-${t.id}`) as HTMLInputElement|null;const n=Number(el?.value);if(n>0)void run(async()=>{await moveConvergenceTeamPosition(t.id,n);setMessage(`Moved ${t.name} to position ${n}.`);});}} className="text-[10px] text-gold">Move</button>{t.status==='ELIMINATED'&&<button disabled={busy} onClick={()=>void run(async()=>{await restoreConvergenceTeam(t.id);setMessage(`Restored ${t.name}.`);})} className="text-[10px] text-green-300">Restore</button>}</div>)}</div><div className="flex gap-2"><button disabled={busy} onClick={()=>void run(async()=>{if(window.confirm('Reopen Checkpoint 1? This rolls the A-D teams back to the Checkpoint 1 wait state.'))await reopenConvergenceCheckpoint(1);})} className="border border-yellow-900 text-yellow-300 px-3 py-2 rounded text-xs">Reopen Checkpoint 1</button><button disabled={busy} onClick={()=>void run(async()=>{if(window.confirm('Reopen Checkpoint 2? This rolls the A-D teams back to the Checkpoint 2 wait state.'))await reopenConvergenceCheckpoint(2);})} className="border border-yellow-900 text-yellow-300 px-3 py-2 rounded text-xs">Reopen Checkpoint 2</button></div></div><div className="flex flex-wrap gap-2"><button disabled={busy} onClick={()=>void downloadExport()} className="bg-gold text-black px-4 py-2 rounded text-sm font-semibold">Export JSON</button><button disabled={busy} onClick={()=>void downloadCsv()} className="border border-gray-700 text-gray-300 px-4 py-2 rounded text-sm">Export Routes CSV</button></div><div className="grid md:grid-cols-2 gap-3"><label className="text-[10px] uppercase text-gray-500">Team history<select value={historyTeam} onChange={e=>setHistoryTeam(e.target.value)} className="mt-1 w-full bg-[#171717] border border-gray-700 rounded px-3 py-2 text-sm text-white"><option value="">Select team</option>{teams.map(t=><option key={t.id} value={t.id}>{t.name} · Track {t.track_id}</option>)}</select></label><button disabled={!historyTeam||busy} onClick={()=>void loadHistory()} className="self-end border border-gold/40 text-gold px-4 py-2 rounded text-sm">Load History</button></div><div className="overflow-auto">{history.length>0&&<table className="w-full text-xs"><thead className="text-gray-500 uppercase"><tr><th className="text-left py-2">Time</th><th>Phase</th><th>Position</th><th>Clue</th><th>Status</th><th>Event</th></tr></thead><tbody>{history.map(h=><tr key={String(h.id)} className="border-t border-gray-900"><td className="py-2">{String(h.assigned_at)}</td><td className="text-center">{String(h.stage_group)}</td><td className="text-center">{String(h.position_at_time??'—')}</td><td className="text-center">{String(h.clue_number??'—')}</td><td className="text-center">{String(h.status)}</td><td className="text-center">{String(h.event_type)}</td></tr>)}</tbody></table>}</div><textarea value={importText} onChange={e=>setImportText(e.target.value)} rows={10} placeholder="Paste a Convergence config JSON export…" className="w-full bg-[#171717] border border-gray-700 rounded p-3 text-white font-mono text-xs"/><button disabled={busy||!importText.trim()} onClick={()=>void importConfig()} className="border border-gold/40 text-gold px-4 py-2 rounded text-sm">Import JSON</button></section>}
