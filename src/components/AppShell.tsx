...
export function AppShell({ children, showBack, showMenu, title, className }: AppShellProps) {
  ...
  return (
    <div className={cn("mobile-app-container flex flex-col relative w-full", className)}>
      {/* Background is now applied via mobile-app-container */}
      ...
      <header className="safe-pt px-4 py-4 flex items-center justify-between sticky top-0 z-40">
        <div className="w-10 flex justify-start">
          {showBack && (
            <button onClick={() => navigate(-1)} className="p-2 -ml-2 text-gold hover:text-gold-hover transition-colors">
        ...
        <div className="flex-1 text-center">
          <h1 className="font-display text-xl md:text-2xl text-offwhite tracking-widest uppercase">
        ...
        <div className="w-10 flex justify-end">
          {showMenu && (
            <button onClick={() => setMenuOpen(true)} className="p-2 -mr-2 text-gold hover:text-gold-hover transition-colors">
      ...
      {/* Main Content */}
      <main className={cn("flex-1 flex flex-col px-6 pb-24 relative z-10", className)}>
        {children}
      </main>
      
      {/* Footer Compass Watermark */}
